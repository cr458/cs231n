...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden_layer_output, hidden_cache = affine_relu_forward (X,W1,b1)
        scores, scores_cache = affine_forward(hidden_layer_output,W2,b2)
        ...
        dw2 += self.reg*W2
        dx,dw1,db1 = affine_relu_backward(dx2,hidden_cache)
        dw1 += self.reg*W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        weight_dict = {'W'+str(i+1):weight_scale*np.random.randn(all_layers[i],all_layers[i+1]) for i in range(len(all_layers)-1)}
        bias_dict = {'b'+ str(i+1):np.zeros(all_layers[i+1]) for i in range(len(all_layers)-1)}
        self.params.update(weight_dict)
        self.params.update(bias_dict)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        if self.use_batchnorm:
            ...
            gammas = {'gamma'+str(i+1):np.ones(all_layers[i+1]) for i in range(len(all_layers)-2)}
            betas = {'beta'+str(i+1):np.zeros(all_layers[i+1]) for i in range(len(all_layers)-2)}
    ...
    def loss(self, X, y=None):
        ...
        data_loss, dscores = softmax_loss(scores,y)
        reg_loss = 0.0
        for weight in [self.params[f] for f in self.params.keys() if f[0] == 'W']:
