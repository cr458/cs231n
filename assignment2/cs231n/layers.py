...
def relu_forward(x):
    ...
    out = None
    ###########################################################################
    # TOD O: Implement the ReLU forward pass.                                  #
    ###########################################################################
    out = np.maximum(0,x)
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (mu, xmu, sq, var, sqrtvar, invvar,
        ...
        #######################################################################
        normalised = (x-running_mean)/np.sqrt(running_var + eps)
        out = gamma*normalised + beta
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    mu, xmu, sq, var, sqrtvar, invvar, va2, va3, gamma, beta, x, bn_param = cache
    eps = bn_param.get('eps', 1e-5)
    N, D = dout.shape
    ...
    # Backprop step 4
    dsq = 1 / float(N) * np.ones((sq.shape)) * dvar
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N, C, H, W = x.shape
    F, C, HH, WW = w.shape
    H_out = 1+(H-HH+2*pad_size)/stride
    W_out = 1+(W-WW+2*pad_size)/stride
    out = np.zeros((N,F,H_out,W_out))
    x_pad = np.pad(x, ((0,), (0,), (pad_size,), (pad_size,)),'constant')
